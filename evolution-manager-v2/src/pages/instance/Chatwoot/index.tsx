...
import { JidFilterPicker } from "@/components/JidFilterPicker";
...
const formSchema = z.object({
  ...
  filterMode: z.enum(["all", "block", "allow"]).default("all"),
...
function Chatwoot() {
  ...
  useEffect(() => {
    // Sin este guard, cualquier refetch en segundo plano (foco de ventana,
    // stale-time vencido) pisa en silencio ediciones sin guardar del usuario
    // via form.reset() — parece que "no se guarda nada" pero en realidad
    // nunca llega a mandarse lo que el usuario tildó.
    if (chatwoot && !form.formState.isDirty) {
      const chatwootData: ChatwootType = {
        ...
        allowedJids: chatwoot.allowedJids || [],
      };

      form.reset(chatwootData);
      form.setValue("filterMode", chatwoot.allowedJids?.length ? "allow" : chatwoot.ignoreJids?.length ? "block" : "all");
    }
  }, [chatwoot, form]);
  ...
  const onSubmit = async (data: FormSchema) => {
    ...
    const chatwootData: ChatwootType = {
      ...
      ignoreJids: data.filterMode === "block" ? data.ignoreJids : [],
      allowedJids: data.filterMode === "allow" ? data.allowedJids : [],
    };
          ...
          <div>
            ...
            <div className="mx-4 space-y-2 divide-y [&>*]:px-4 [&>*]:py-2">
              ...
              <div className="space-y-2 py-2">
                <span className="text-sm font-medium">Privacidad — qué chats se envían a Chatwoot</span>
                <div className="flex gap-4">
                  {(
                    [
                      { value: "all", label: "Todos" },
                      { value: "block", label: "Excluir seleccionados" },
                      { value: "allow", label: "Permitir solo seleccionados" },
                    ] as const
                  ).map((option) => (
                    <label key={option.value} className="flex items-center gap-1.5 text-sm">
                      <input
                        ...
                        onChange={() => form.setValue("filterMode", option.value, { shouldDirty: true })}
                      />
                      {option.label}
                    </label>
                  ))}
                ...
                {filterMode !== "all" && (
                  <p className="text-xs text-muted-foreground">
                    {filterMode === "block"
                      ? "Los tildados NO se sincronizan a Chatwoot."
                      : "Solo los tildados se sincronizan a Chatwoot."}
                  </p>
                )}
                {/*
                  El picker queda visible en cualquier modo — no solo sirve para elegir
                  el filtro de privacidad, tambien es el unico lugar donde crear la
                  conversacion de un canal en Chatwoot (boton "Crear en Chatwoot" en
                  Canales), algo que no depende de que filtro este activo.
                */}
                <JidFilterPicker
                  instanceName={instance?.name}
                  selectable={filterMode !== "all"}
                  values={filterMode === "all" ? [] : filterMode === "allow" ? form.watch("allowedJids") || [] : form.watch("ignoreJids") || []}
                  onValuesChange={(values) => {
                    if (filterMode === "all") return;
                    form.setValue(filterMode === "allow" ? "allowedJids" : "ignoreJids", values, { shouldDirty: true });
                  }}
                />
              </div>
